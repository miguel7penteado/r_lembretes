...
{
	dados_instantaneos <- eventReactive(input$botao_r , { rnorm(input$objeto_r_entrada) })	
	
	output$meu_objeto_r <- renderPlot({ hist( dados_instantaneos(), main = isolate(input$minha_caixa_texto) ) })
